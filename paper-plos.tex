...
Also, due to  lack a standard FASTQ record id, which is
...
FASTQ format have to be preprocessed (referred to as~$T_{comm}$)
...
A popular way to define and run bioinformatics pipelines on HPC resources
is via the Galaxy\cite{galaxy} platform, which provides a Web-based graphical
...
for both scenarios. We found that Hadoop becomes comparable with HPC when processing
...
improved by redefining the queueing system's default behavior. Finally, we demonstrated that an existing and extensible
