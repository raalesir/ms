...
\begin{flushleft}
{\Large
\textbf{A quantitative assessment of the Hadoop \textcolor{blue}{and HPC} framework for analysing massively parallel DNA sequencing data}
}
...
Many of the most widespread tools for this purpuse, like BWA~\cite{bwa}, Bowtie~\cite{Langmead:2009uq} and Samtools~\cite{samtools}, are not made with distributed computing in mind. Many others do not even have the native ability to use multiple cores. 
...
The most prevalent open source implementation of Map-Reduce is Hadoop~\cite{hadoop,Hadoop:Guide}. \textcolor{blue}{Please clarify, I checked the bib file and... found nothing...}\COMMENT{Fix bib error (can't have both editor and author for this ref type)} 
...
All experiments were repeated several times and the results averaged \textcolor{blue}{to use medians I had to run much more experiments... each takes 10 hours .... not possible. } \COMMENT{considering the variations in table 4, why not use medians} to obtain a data point for the particular combination of data size and computational platform.
...
The requirement is that the FASTQ data are i.e. BZIP2 archived\COMMENT {there was a footnote about LZO. Why was is bad?}, (however, any splittable archive format is suitable) and accessible by Hadoop. For our case the storage with the data was mounted with SSHFS to one of the Hadoop nodes, from where the BZIP2'ed data were transferred to the HDFS.
...
To complete the comparison the same preprocessing was performed with a BASH script for the same data located on the HPC storage. The script (see Supplementary information) ran on a single HPC node, utilizes multiple cores, with the limiting factor being \sout{the hard disk IO performance.}\COM{More correctly to say, that the pipe is not filled completely. i.e. the degree of achieved parallelization does not utilize all 16 cores. Perhaps not the best bash commands etc.}
...
To demonstrate the scalability of the HPC approach we collected running times for dataset~I as a function of the number of cores used (Table~\ref{table:3}). \COMMENT{Consider replacing with a graph.} \COM{no, lets leave it as a table... with the better caption.}
The aligning \COMMENT{mapping written in table, be consistent} process in Bowtie scales fairly well, but the SNP calling part in Samtools is a bottleneck, scaling worse than Bowtie, and consuming a progressively larger portion of the total calculation time even after parallelising analysis by chromosome  as suggested at~\cite{biostars_samtools}.
...
The parabolic extrapolation for our settings give the ratio as~$1.70\pm0.01$, meaning that Hadoop running even in the virtualized environment of a private cloud assembled on moderate hardware, is competitive with the HPC approach run on ``bare metal'' of modern hardware for datasets greater than $100$\,Gb (dataset~IV), which is typical for human genome sequencing with a sequencing depth of about~$30x$.
...
\caption{Timings (in minutes) for the sort routine of the Samtools package for the dataset IV (58 Gb BAM file). The measurements are done on a single HPC node with $p=16$ cores. Averaging is done over 5 independent simulations. The amount of RAM is given for all 16 cores. }
\begin{center}
...
\textit{Hadoop results (filled circles in Figure~\ref{fig:fig3})}: One can see that the ratio~$T_{mapping}/T_{comm}$ reveals very weak dependency in a wide range of number of nodes~$N$: from 4 up to 40. It is known that Bowtie provides almost linear scaling between mapping time and dataset chunk size~$D$:~$T_{mapping}\propto  D\propto 1/N$, see~\cite{Langmead:2009uq}, and Figure~\ref{fig:fig1}. Since the ratio~$T_{mapping}/T_{comm}$ is approximately constant, one can conclude that $T_{comm}\propto 1/N$, meaning that the more nodes involved, the faster the communication is in the preprocessing stage.
...
%\footnote{The used storage at UPPMAX is a set of RAIDs (Redundant Array of Inexpensive Disks) with data striping, providing up to $80$Gbit/sec of outcoming traffic \COMMENT{'outgoing' or 'incoming' or just 'traffic'}.} 
...
\begin{figure}[!ht]
	...
	\normalsize
	\caption{Ratios of the mapping time~$T_{mapping}$ to the communication costs~$T_{comm}$ for HPC and Hadoop clusters for Dataset~IV as a function of reciprocal number of nodes~$1/N$. Two HPC scenarios are shown as ``HPC SLURM'' and ``HPC random'', which correspond to standard SLURM behaviour and a modified one where nodes are being allocated from random racks.
...
For  the ``HPC SLURM'' $(a,b)\approx (0.96, 3.49)$ and $(a,b)\approx (32.99, 0.60)$, and for  the ``HPC random'' $(a,b)\approx (0.88, 3.72)$ and $(a,b)\approx (98.24, 0.53)$.
 The constant term for HPC goes below unity, i.e. for the large number of nodes the communication will take more time than actual mapping. 
...
Dataset			&		II (7 Gb )	& III (30 Gb)	& IV (100 Gb)\\
\hline
...
data, Gb		&	1.4	&	3.5		&	7.0		&	15.0		&	30.0		&	100.0	&	250 	\\
