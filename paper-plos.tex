...
\Change{%
All data was standardized to FASTQ format and compressed with \texttt{bzip2},
since it is typical to store sequencing data in a compressed file format. The
\texttt{bzip2} compression codec was selected it provides very good compression
...
Also, since some of the datasets lack a standard FASTQ record id, which is
required by our test pipelines to extract the read number, we also standardised
...
Nevertheless, in practice this approach is rather error-prone and laborious.
...
To route both reads of the same pair to the same Reducer, a secondary
key-sorting mechanism was employed.
...
Hadoop-based approach imposes the requirement that the input data be stored
where the Hadoop nodes can access it, which is in our opinion a reasonable since
sequencing platforms generally have direct access to extensive storage and
...
(Methods).  Our experiments consisted of executing the pipelines for each of the
five input 
datasets (see Table~\ref{table:datasets}) and
measuring the wall-clock run time for each pipeline stage.  All experiments were
repeated
\Fixme{several times (how many?)}
and the results averaged to obtain a data point for the
particular combination of data size and computational platform.
}

\Fixme{Belongs with Discussion:
In light of our observations, we
propose modifications for existing Hadoop software to better exploit the
massively parallel nature of MR computations. Finally, we developed a set of Bash
scripts to parallelize the operations of classical DNA-seq analysis programs for
short-read alignment over multiple nodes and fully exploit the network bandwidth, thereby
...
SOAPsnp~\cite{soapsnp}, respectively) we could not expect them to deliver perfectly
...
Section~\ref{sec:preprocessor}  to the original preprocessor furnished with
...
that uses of all processing cores available on a single computing node to accelerate the operation (see
...
\Fixme{Alexey, please verify whether my interpretation seems reasonable. LP}

\Change{%
Table~\ref{table:preprocess} shows the results of the experiment. Clearly,
either of the parallel approaches are much faster than the original tool, with
...
the largest dataset taking requiring less than half the time of the Bash
...
important to understand the scaling characteristics of each processing
platforms (Hadoop and HPC) with respect to input size. For each platform, we
measured the time required for to execute their respective variant calling
pipeline (see Methods) for each of the test datasets (I-V). The results are
shown in Table~\ref{table:4}.

The measurements highlight that one of the advantages of using the Hadoop~MR} framework is
its almost linear scalability with respect to input size -- i.e.\ the
calculation time linearly depends on the size of the
dataset~\cite{Langmead:2009kx,Pireddu:2011vn}.
%Deleting this since one could dream/find pathological cases that don't work: ``
\Fixme{\sout{regardless the scaling nature of the underlying program.}}
Figure~\ref{fig:fig1}, based on Table~\ref{table:4}, shows the wall clock
...
\Change{%
Analogously, we investigated the scalability characteristics of the HPC method
with respect to input size.
\Fixme{The following works better the aid of a plot.}
Analyzing the observations in Table~\ref{table:4}, we can see with the HPC
method run time increases superlinearly as a function of dataset size.  As
expected, the result is that the HPC method is not able to complete the analysis
with our larger tests datasets.  On the other hand, this approach is not much
slower than our Hadoop cluster considering the much smaller quantity of computing
resources that is uses -- an issue that is analyzed more closely in the section
on the efficiency of the two systems.
}

\Fixme{The following paragraph comments on how the HPC approaches' runtime
varies with the number of cores available, rather than input size. If we're
going to leave it in, I think we should make an analogous discussion regarding
how Hadoop's runtime varies with increasing resources. The data in Table 6 might
be suitable for this.}

To investigate the scalability characteristics of the HPC approach we ran the
HPC pipeline using Dataset~I as input. We collected the running times as a
function of the number of cores used (Table~\ref{table:3}). The results show that the
...
\Change{%
Hadoop was designed to process huge datasets by distributing computations to a
large number of computers\cite{hadoop,lin2010}.  Therefore, while allowing us to
process very large datasets, the mechanisms required for distributing the work
add a non-trivial overhead to the computation, which becomes more and more
significant as the amount of work to be done is reduced -- i.e.\ the dataset
size decreases.

In order to measure the overhead incurred by the Hadoop platform, we compared
the efficiency of the two processing methods (Hadoop and HPC) as the size of the
dataset varies. We measure the efficiency indirectly as the number of core-hours
($F$) used for same analysis: $F=T_{p}\times p,$ where~$T_{p}$ is the running time on~$p$ cores.
}

\Change{%
\Fixme{The way of reorganized this text, at the end of the subsection it kind of
excessively teeter-totters too much between HPC and Hadoop.}

Using the data from Table~\ref{table:4}, in Figure~\ref{fig:fig2} we plot the
ratio $F_{Hadoop}/F_{HPC}$ as a function of the reciprocal dataset size.
%
The simplest curve that fits the data points nicely is a parabolic function
which, importantly, is not a constant.  Since our measurements using the
Hadoop-based pipeline, summarized in Figure~\ref{fig:fig1}, showed linear
scaling properties of Hadoop in this setting, a deviation of the ratio
$F_{Hadoop}/F_{HPC}$ from the constant is evidence of the lack of scalability of
the HPC approach.
}%
...
and time-demanding than the alignment with Bowtie.  Since its memory
requirements scale with input size, with sufficient input data Samtools swaps
data to disk, creating multiple (up to hundreds) temporary files while it sorts
the BAM file -- incidentally, these files can end up on shared network-accessed
file systems and thus generate large amounts of network traffic.
Table~\ref{table:5} shows the time required by Samtools to sort dataset~IV with
...
on the cluster network -- the trend quite clearly indicates that a relatively
large amount of RAM per node is needed in order to use Samtools effectively. In
contrast, in our tests Hadoop performed the SNP calling on much more economic
hardware, in linear time, with just 12GB of RAM per \textit{virtual} node.

\Change{%
Nevertheless, it is evident from the plot that the HPC method is always more
efficient in its use of CPU time than the Hadoop-based approach, and thus takes fewer computing
resources to perform the same amount of work.  However, Hadoop becomes
increasingly more efficient with respect to the HPC method as the dataset size
increases.
}
...
\Change{%
In addition it is quite important to note that the price paid for
the efficiency in HPC's method is in overall runtime and in its inability to
finish the analysis on Dataset~V in a reasonable time (over 48 hours to Hadoop's 19.4
hours; see Table~\ref{table:4}).
}
...
splitting the initial data into chunks to processing them independently. In this
approach, reducing the size of each data chunk reduces the size of the aligner
job,~$T_{alignment}$ but, at the same time, the more chunks almost
simultaneously have to be sent over the network, potentially causing traffic
jams, and therefore increasing the communication costs,~$T_{comm}$.
...
\Fixme{Galaxy seems a bit out of place to me.
\sout{A popular way to define and run bioinformatics pipelines on HPC resources
is via the} Galaxy\cite{galaxy} \sout{platform, which provides a Web-based graphical
...
The Cloudgene\cite{cloudgene} GUI is available to help lower the learning curve
...
data and parameters for its execution. Most of the data managing work is done
...
pressing a button and entering the credentials; at the same time, the can be
...
on Hadoop. We show that Hadoop installed on a private cloud is an appealing
solution both on terms of runtime performance and also in terms of usability. A
key result is the data size where Hadoop becomes an advantageous option as compared
to regular HPC batch systems.
In order to compare the two platforms we developed highly optimized pipelines
for both scenarios. We found that Hadoop excels over HPC when processing
datasets larger than 100\,Gbases. Also, Hadoop shows almost linear scalability for
this problem. Extrapolation to infinite dataset size reveals however that HPC
provides the results faster, given the same amount of resources as Hadoop on a
private cloud. \Fixme{But the HPC method tested ran on a single node so it
didn't incur distribution costs!}
...
\begin{verbatim}
pbzip2 -dc  \${file1} | paste - - - - -d'\textbackslash t' | cut -f1,2,4 | paste - -d' ' <(pbzip2 -dc  \${file2} | paste - - - - -d'\textbackslash t' | cut -f2,4) | pbzip2 -cz > \${fileOut}
\end{verbatim}
...
\begin{figure}[!ht]
	...
	\Change{%
		...
		\Fixme{\\ What would you think of inserting line plotting the 16-core HPC
		tests from Table 4}
		}
	\label{fig:fig1}
\end{figure}
...
\caption{%
	\Change{%
	Timings for alignment and the ratio~$T_{alignment}/T_{comm}$ for HPC and
	Hadoop clusters for Dataset IV\@.  For the ``HPC random'' approach, data
	chunks have to be copied to the local scratch disks first and the output alignments
	(SAM files) copied back. On the contrary, Hadoop keeps all the data inside HDFS and hence
	does not need data staging. However, Hadoop needs to preprocess reads before
	the actual alignment stage to group read mates into the same record,
	resulting in what we term ``communication costs''. Note that each HPC node has
	16 cores, while each Hadoop node has 7 (one core is dedicated to run the
	virtual machine).
	}
}
