...
% LP: I think this sentence could be removed since to appreciate what is being
% described one would have to understand quite well how that program works.
...
Since our HPC and Hadoop pipelines use different~SNP callers (Samtools and SOAPsnp~\cite{soapsnp}, respectively) we do not expect them to deliver perfectly matching SNP lists, but we do expect them to capture and correctly identify the
% which mutation?
\Fixme{mutations}.
We only tested for correctness with the smallest dataset~(I). The mutation $C\rightarrow T$ on chromosome 4 at position $16702262$~\cite{schneeberger} was successfully localized by both variant callers.



\subsection*{Scalability of HPC and Hadoop approaches}
\Change{To demonstrate the scalability of the HPC approach we ran the HPC pipeline using dataset~I as input. We collected the running times} as a function of the number of cores used (Table~\ref{table:3}).
We found that the alignment process with Bowtie scales fairly well, while SNP calling with Samtools is the bottleneck of the workflow. Indeed, this latter part scales worse than Bowtie and consumes a progressively larger portion of the total calculation time as the input grows, even after parallelizing analysis by chromosome as previously suggested~\cite{biostars_samtools}.
...
\Change{For both the HPC and Hadoop approaches, and using all the test datasets (I-V), we collected the time required for the combined task of mapping the reads to the corresponding reference genome and SNP calling. The results are shown in Table~\ref{table:4}.}

\Change{The results confirm that one the advantages to using the Hadoop~MR} framework is its almost linear scalability -- i.e.\ the calculation time linearly depends on the size of the dataset~\cite{Langmead:2009kx,Pireddu:2011vn}. \Fixme{Deleting this since one could dream/find pathological cases that don't work: ``regardless the scaling nature of the underlying program''.}
Figure~\ref{fig:fig1}, based on Table~\ref{table:4}, shows the calculation time as a function of the dataset size for the 56-core Hadoop cluster and datasets I-IV.
...
\Change{%
Hadoop was designed to process huge datasets\cite{hadoop,lin2010}. Therefore, one could image then that the larger the dataset the more advantageous Hadoop should become compared to the HPC approach. In order to compare the efficiency of the two processing platforms (Hadoop and HPC) as the size of the dataset varied, we looked at the number of core-hours used for same analysis:
$F=T_{p}\times p,$ 
where~$T_{p}$ is the calculation time on~$p$ cores. 
}

\Change{%
Using the data from Table~\ref{table:4}, in Figure~\ref{fig:fig2} we plot the ratio $F_{Hadoop}/F_{HPC}$ as a function of the reciprocal dataset size. Extrapolation to zero on the~$X$-axis estimates the ratio for the hypothetical infinite dataset. It is evident from the data that the Hadoop pipeline becomes increasingly more efficient than the HPC-based pipeline as the dataset size increases. 
The parabolic extrapolation of our data yields the value~$1.70\pm0.01$ for the ratio, meaning that Hadoop, even if running in the virtualized environment of a private cloud running on modest hardware, is competitive with the HPC approach run on ``bare metal'' of modern hardware for datasets larger than $100$\,Gbases (dataset~IV), which is typical for human genome sequencing with a sequencing depth of about~$30x$.
}


\Change{%
The simplest curve that fits the data points nicely is a parabolic function which, importantly, is not a constant. 
Since our measurements using the Hadoop-based pipeline, summmarized in Figure~\ref{fig:fig1}, showed linear scaling properties of Hadoop in this setting, a deviation of the ratio $F_{Hadoop}/F_{HPC}$ from the constant is evidence of the 
...
We suspect the main reason for such a behavior is that for large datasets (dataset~III-V, in our case) Samtools' SNP calling routine becomes more memory- and time-demanding than the alignment with Bowtie.
Since its memory requirements scale with input size, with sufficient input data Samtools swaps data to disk, creating multiple (up to hundreds) temporary files while it sorts the BAM file -- incidentally, these files can end up on shared network-accessed file systems and thus generate large amounts of network traffic.
Table~\ref{table:5} shows the time required by Samtools to sort dataset~IV with different amount of RAM available on a single HPC node. Though there is a large deviation when using small amounts of RAM, we believe due to other users' load on the cluster network, the trend quite clearly indicates that a relatively large amount of RAM per node is needed in order to use Samtools effectively. In contrast, in our tests Hadoop performed the SNP calling on much more economic hardware in linear time with just 12GB of RAM per \textit{virtual} node.
}
